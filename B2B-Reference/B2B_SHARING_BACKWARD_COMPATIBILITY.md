...
| `SHARE_GRANT`, `SHARE_GRANT_EXCLUSION`, `SHARE_GRANT_EDITABLE_FIELD`, `RESOURCE_OVERLAY` tables | Did not exist | New tables, empty until shares are created | No | None (§4) |
...
(`resolveActingOUID`, `backend/internal/role/assignment_service.go:84-91`), so backfilling
...
(`assignment_service.go:406-473`) resolves `actingOUID := resolveActingOUID(ouID, role.OUID)` and
calls `requireOwnOUScope(ctx, actingOUID)` — requiring the caller's own OU to match, unless it holds
the deployment's root `system` permission or the request runs under a runtime context. Where
`actingOUID` differs from the role's own OU, it additionally requires
`sharingService.IsShared`/`ResolveEditability`.
...
`POST /roles/{id}/share-grants`. Fix: either issue that caller a token with root `system`
permission, or set up the appropriate share grant so it becomes a legitimate sharee.
...
`SHARE_GRANT`, `SHARE_GRANT_EXCLUSION`, `SHARE_GRANT_EDITABLE_FIELD`, and `RESOURCE_OVERLAY` did not
exist on `main`. They have no foreign key into or out of any pre-existing table (in particular, no
FK from `SHARE_GRANT` into `ROLE`/`ROLE_ASSIGNMENT` — the sharing framework is deliberately generic
