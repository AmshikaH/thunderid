...
owner (unconditional), or the tree's root with an incoming Share grant, or does some earlier
...
The framework tables (`SHARE_GRANT`, `SHARE_GRANT_EXCLUSION`, `SHARE_GRANT_EDITABLE_FIELD`,
...
Role's `assignments` templated field is the one exception: it's stored directly in the existing
`ROLE_ASSIGNMENT` table (extended with an OU column) rather than in the generic `RESOURCE_OVERLAY`
table. This is deliberate, not a shortcut: assignments are relational (many rows per role, not a
scalar value), and the authorization hot path needs a single indexed SQL predicate against a table
it already joins, not JSON parsing out of a generic overlay table on every permission check. The
generic sharing/policy tables themselves stay entirely free of role-specific schema — this is the
only place a resource type's own storage is reused instead of the generic path, and it's confined
to Role's own pre-existing table.
...
- `SHARE_GRANT.TIER` didn't say *what* it distinguished (the two-step share/reshare process) →
  renamed to `SHARE_STAGE`.
- `SHARE_GRANT.TARGET_KIND` didn't convey that it's a *breadth* concept (a single OU vs. an "all"
...
enum values, exclusions are modeled as a separate table (`SHARE_GRANT_EXCLUSION`) attached to an
...
own resource collection, `/roles/{id}/share-grants` (`GET`, and `DELETE .../{grantId}`). This also
...
**Fix:** Reshare no longer requires a prior Share grant when the resharing OU *is* the resource's
...
.../share-grants`), which reported "reshare" for what was really the owner's own first share.
...
assignment-management capability the share grant was supposed to hand it — which is close to never,
...
### 5.22 Export/Import and Declarative-YAML Share Grant Round-Tripping

**The problem:** exporting a role (declarative export, `roleExporter.GetResourceByID`) captured
core config, permissions, and the owning OU's own assignments — but not the role's share grants,
...
representation for a share grant at all — there was no way to hand-author "this bootstrap role is
...
visible. Every one of export (round-trip verification), declarative-YAML `shareGrants:`, and
`POST /import`'s `shareGrants` calls the identical `Share()` method a live
`POST /roles/{id}/share-grants` request does; a hand-authored grant, an exported grant, and a live
...
**`sharedAssignments` is rejected outright for declarative (file-store) roles.** This is not a gap
to close later; it is a real, pre-existing constraint surfacing through a new door.
`internal/role/file_based_store.go`'s `AddAssignments`/`RemoveAssignments` unconditionally return
`errors.New("AddAssignments is not supported in file-based store")` (and the equivalent for
`RemoveAssignments`) — sibling read methods on the same store already document, in their own
comments, that declarative roles are not shareable for assignment purposes. A declarative role's
assignments live in its YAML file, not a mutable per-OU table; there is no per-sharee-OU assignment
set to hold in the first place. `loadDeclarativeResources`'s `applyPendingShares` therefore fails
server startup outright (a hard error, not a silently-dropped field) if a declarative role's YAML
declares `sharedAssignments`, while `shareGrants` is fully supported there — a declarative role can
be *pre-shared*, it just can't hold per-sharee assignment state until it becomes a real, DB-backed
role. `POST /import` has no such restriction: it always targets real DB-backed roles (via
`roleService.CreateRole`/`UpdateRoleWithPermissions`), which do support sharee-OU assignment
writes, so `applyRoleSharing` there applies both `shareGrants` and `sharedAssignments`
unconditionally.
...
configuration error (`"sharingService not configured"`) rather than silently skipping share/
assignment replay, consistent with how every other optional adapter on `importService` is guarded.
...
| A new schema column recording which OU issued each reshare hop | Unnecessary: because explicit hops are now restricted to immediate children, the issuer is always derivable at read time from the OU tree itself (the target's real parent), so nothing needs to be stored beyond what `SHARE_GRANT` already has. |
