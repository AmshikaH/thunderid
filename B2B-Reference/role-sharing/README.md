...
templated-field editability as a property of the share grant itself — independently per assignee
...
sharing and resharing are issued through the single `POST /roles/{id}/share-grants` endpoint (a
...
├── 11 - Chained Delegation and Chain Integrity       # One-hop-at-a-time delegation; exclusion cuts off grandchildren
...
- A fourth, throwaway **Export Demo Role** (`15`) is created, shared to B's own children, given a
  ...
  back, to prove the round trip restores both the share grant and the sharee assignment rather than
...
5. **04 - Reshare into Root's Subtree**: reshares with `allChildren: true` and confirms both the
   ...
   account for sharing rather than listing only owned roles.
...
16. **15 - Export/Import Round-Trip**: on a fresh throwaway role owned by B, shares directly to
    ...
    returned YAML includes both `shareGrants:` and `sharedAssignments:` for the role, not just its
    core config. It revokes the grant (the export already captured a point-in-time snapshot, so
    this doesn't affect it) and deletes the role, then calls `POST /import` with the exported
    content unchanged. The import recreates the role under its original id, replays the share
    grant through the same `Share` call a live `POST /roles/{id}/share-grants` request would use,
    and reapplies B-sub1's assignment, both confirmed by a follow-up `GET .../share-grants` and
    `GET .../assignments?ouId=`. Cleans up its own grant and role at the end of the folder.
