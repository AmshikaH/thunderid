...
| **OU Select** | An organization unit selection field. Renders as a flat dropdown of an OU's child handles for the OU Resolver's `prompt` strategy, or an expandable tree of every OU (selected by ID) for its `promptAll` strategy. See "Resolve OU" under [Identity and User Resolution](#identity-and-user-resolution) for the OU Resolver's strategy details. |
...
- **Failure conditions:** OU service error; selected value does not match any OU ID in the tree

**Input Configuration:**

The `prompt` strategy accepts exactly one of the two inputs below, never both:
- `ouId`: a literal OU ID, for a caller that already knows the target OU. Default: `ouId`
- `ouHandle`: a handle scoped to the default OU's children, as offered by the OU selection component. Default: `ouHandle`

The `promptAll` strategy always uses:
- `ouId` (required): the selected OU's ID. Default: `ouId`
