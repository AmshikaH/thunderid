...
-- when the role has been shared to that OU (see SHARE_GRANT). This is what makes authorization
...
-- Table capturing the resource-sharing graph. Generic across resource types: a SHARE_GRANT row
...
-- (TARGET_SCOPE='all_children') are NOT snapshotted; TARGET_OU_ID holds the anchor root, and
-- subtree membership is evaluated dynamically at read time by walking the target OU's ancestor
-- chain up to that anchor, so newly created child OUs are automatically in scope with no backfill.
CREATE TABLE "SHARE_GRANT" (
    ...
    TARGET_SCOPE    VARCHAR(12) NOT NULL CHECK (TARGET_SCOPE IN ('all_roots', 'root', 'all_children', 'ou')),
    TARGET_OU_ID    VARCHAR(36),
    PARENT_GRANT_ID VARCHAR(36) REFERENCES "SHARE_GRANT" (ID) ON DELETE CASCADE,
    CREATED_AT      TIMESTAMPTZ DEFAULT NOW(),
    UPDATED_AT      TIMESTAMPTZ DEFAULT NOW()
);

-- Supports "list grants for a resource" and the anchored visibility lookup (resource + target).
CREATE INDEX idx_share_grant_resource ON "SHARE_GRANT" (DEPLOYMENT_ID, RESOURCE_TYPE, RESOURCE_ID);
CREATE INDEX idx_share_grant_target ON "SHARE_GRANT"
...
CREATE TABLE "SHARE_GRANT_EXCLUSION" (
    DEPLOYMENT_ID  VARCHAR(255) NOT NULL,
    GRANT_ID       VARCHAR(36) NOT NULL REFERENCES "SHARE_GRANT" (ID) ON DELETE CASCADE,
...
CREATE INDEX idx_share_grant_exclusion_grant ON "SHARE_GRANT_EXCLUSION" (DEPLOYMENT_ID, GRANT_ID);

-- Per-grant materialized set of templated field keys editable through that grant, fixed at grant
-- creation time (see ShareGrant.EditableFields): every declared field, for a grant issued by the
...
CREATE INDEX idx_share_grant_editable_field_grant ON "SHARE_GRANT_EDITABLE_FIELD" (DEPLOYMENT_ID, GRANT_ID);

-- Generic per-(resource, OU) templated field overlay. Default backing store for a resource type's
-- templated fields; a resource type may opt a specific field out to a specialized store instead
-- (e.g. role assignments use ROLE_ASSIGNMENT directly for relational/authz-path performance) —
-- see the sharing package's TemplatedFieldStore for that seam. No resource-type-specific tables
-- are added for sharing/overrides beyond this generic set.
CREATE TABLE "RESOURCE_OVERLAY" (
    ...
    UPDATED_AT      TIMESTAMPTZ DEFAULT NOW(),
    PRIMARY KEY (RESOURCE_TYPE, RESOURCE_ID, OU_ID, FIELD_KEY, DEPLOYMENT_ID)
);
