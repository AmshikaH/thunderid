...
// (optionally narrowed by an exclusion list — see ShareGrant.ExcludedOUIDs).
...
// ShareGrant is one row of the sharing graph: either a share-stage grant (owning OU -> Root(s))
...
type SharePolicy struct {
	...
	// resource.
	OUIDs []string
	// ExcludedOUIDs carves these OUs (and their subtrees) out of the AllChildren grant. Each must
	// be the caller itself or within its subtree. Ignored unless AllChildren is true.
	ExcludedOUIDs []string
...
type TemplatedFieldDeclaration struct {
	...
	Key string
	// FallbackKey, when non-empty, names a coarser sibling field consulted when a share grant's
