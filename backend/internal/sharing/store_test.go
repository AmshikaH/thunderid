...
func (suite *StoreTestSuite) TestCreateGrant() {
	...
	suite.mockDBClient.On("ExecuteContext", mock.Anything, queryCreateShareGrant,
		"grant1", "role", "role1", "owner-ou", "share", "root", "root1", nil, testStoreDeploymentID,
	).Return(int64(1), nil)

	err := suite.store.CreateGrant(context.Background(), ShareGrant{
...
func (suite *StoreTestSuite) TestGetGrant_NotFound() {
	suite.mockDBProvider.On("GetConfigDBClient").Return(suite.mockDBClient, nil)
	suite.mockDBClient.On("QueryContext", mock.Anything, queryGetShareGrantByID, "missing", testStoreDeploymentID).
...
func (suite *StoreTestSuite) TestGetGrant_Found() {
	suite.mockDBProvider.On("GetConfigDBClient").Return(suite.mockDBClient, nil)
	suite.mockDBClient.On("QueryContext", mock.Anything, queryGetShareGrantByID, "grant1", testStoreDeploymentID).
...
func (suite *StoreTestSuite) TestDeleteGrant() {
	suite.mockDBProvider.On("GetConfigDBClient").Return(suite.mockDBClient, nil)
	suite.mockDBClient.On("ExecuteContext", mock.Anything, queryDeleteShareGrant, "grant1", testStoreDeploymentID).
		Return(int64(1), nil)
...
func (suite *StoreTestSuite) TestOverlay_GetSetDelete() {
	...
	suite.mockDBClient.On("QueryContext", mock.Anything, queryGetResourceOverlay,
		"role", "role1", "ou1", "assignments", testStoreDeploymentID).
		Return([]map[string]interface{}{{"value": `["u1"]`}}, nil).Once()
	value, found, err := suite.store.GetOverlay(context.Background(), "role", "role1", "ou1", "assignments")
	suite.NoError(err)
	suite.True(found)
	suite.Equal(`["u1"]`, value)

	suite.mockDBClient.On("QueryContext", mock.Anything, queryGetResourceOverlay,
		"role", "role1", "ou1", "missing", testStoreDeploymentID).
		Return([]map[string]interface{}{}, nil).Once()
	_, found, err = suite.store.GetOverlay(context.Background(), "role", "role1", "ou1", "missing")
	suite.NoError(err)
	suite.False(found)
	...
	suite.mockDBClient.On("ExecuteContext", mock.Anything, queryUpsertResourceOverlay,
		...
		Return(int64(1), nil)
	suite.NoError(suite.store.SetOverlay(context.Background(), "role", "role1", "ou1", "assignments", `["u1","u2"]`))

	suite.mockDBClient.On("ExecuteContext", mock.Anything, queryDeleteResourceOverlay,
		"role", "role1", "ou1", "assignments", testStoreDeploymentID).
		Return(int64(1), nil)
	suite.NoError(suite.store.DeleteOverlay(context.Background(), "role", "role1", "ou1", "assignments"))
}
...
func (suite *StoreTestSuite) TestCreateGrant_ProviderError() {
	...
	err := suite.store.CreateGrant(context.Background(), ShareGrant{ID: "grant1"})
...
func (suite *StoreTestSuite) TestSetOverlay_ProviderError() {
	...
	err := suite.store.SetOverlay(context.Background(), "role", "role1", "ou1", "assignments", "v")
...
func (suite *StoreTestSuite) TestDeleteOverlay_ProviderError() {
	...
	err := suite.store.DeleteOverlay(context.Background(), "role", "role1", "ou1", "assignments")
