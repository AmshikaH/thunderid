...
func (as *roleAssignmentService) mutateAssignments(
	...
	normalized, actingOUID, svcErr := as.prepareAssignments(ctx, id, ouID, assignments)
	if svcErr != nil {
		return svcErr
	}

	if err := as.transactioner.Transact(ctx, func(txCtx context.Context) error {
		return storeFn(txCtx, id, actingOUID, normalized)
	}); err != nil {
...
// prepareAssignments validates the request, resolves the OU the caller is acting as (the role's
// own OU, or a sharee OU's ID), and verifies that OU is allowed to write assignments: the owning
// OU always may; any other OU must currently have a valid share/reshare grant for the role, with
// assignments of every type present in the request editable for it (editability is controlled
// independently per assignee type — see roleResourceTypeDeclaration). Without this check, any OU
// could write itself permissions via ROLE_ASSIGNMENT with no sharing relationship at all.
func (as *roleAssignmentService) prepareAssignments(
	ctx context.Context, id, ouID string, assignments []RoleAssignment,
) ([]RoleAssignment, string, *tidcommon.ServiceError) {
	logger := log.GetLogger().With(log.String(log.LoggerKeyComponentName, assignmentLoggerComponentName))
	...
	// both be entitled to act as actingOUID at all (this check) and, when acting as a sharee, have
	// a valid share grant with the field made editable (the checks that follow).
	if svcErr := requireOwnOUScope(ctx, actingOUID); svcErr != nil {
		return nil, "", svcErr
	}

	if actingOUID != role.OUID {
		shared, svcErr := as.sharingService.IsShared(ctx, roleSharingResourceType, id, actingOUID)
		if svcErr != nil {
		...
		if !shared {
			logger.Debug(ctx, "Role is not shared to the acting OU",
				log.String("id", id), log.String("ouID", actingOUID))
			return nil, "", &ErrorRoleNotSharedToOU
		}
		for _, assigneeType := range distinctAssigneeTypes(assignments) {
			fieldKey := assignmentsFieldKeyForType(assigneeType)
			editable, svcErr := as.sharingService.ResolveEditability(
				ctx, roleSharingResourceType, id, role.OUID, actingOUID, fieldKey)
			if svcErr != nil {
	...
	return normalized, actingOUID, nil
}
