...
func (f *fakeSharingService) ListShareGrants(
	ctx context.Context, resourceType sharing.ResourceType, resourceID string,
) ([]sharing.ShareGrant, *tidcommon.ServiceError) {
	if f.listShareGrantsFunc != nil {
		return f.listShareGrantsFunc(ctx, resourceType, resourceID)
	}
	panic("fakeSharingService: unexpected call to ListShareGrants")
}
...
func (suite *RoleServiceTestSuite) TestGetRolesForOU_SkipsStaleSharedRole() {
	...
	suite.Nil(err)
	suite.Empty(result.Roles, "a share grant pointing at a since-deleted role should be skipped, not error")
}
