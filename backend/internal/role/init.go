...
func Initialize(
	...
	if fileStore != nil {
		if err := loadDeclarativeResources(fileStore, dbStore, roleService, sharingService); err != nil {
			return nil, nil, nil, nil, err
		}
	}
...
func registerRoutes(mux *http.ServeMux, roleHandler *roleHandler) {
	...
	mux.HandleFunc(middleware.WithCORS(
		"GET /roles/{id}/share-grants", roleHandler.HandleRoleShareGrantsGetRequest, opts6))
	mux.HandleFunc(middleware.WithCORS(
		"POST /roles/{id}/share-grants", roleHandler.HandleRoleShareGrantsPostRequest, opts6))
	mux.HandleFunc(middleware.WithCORS(
		"DELETE /roles/{id}/share-grants/{grantId}", roleHandler.HandleRoleUnshareRequest, opts6))
	mux.HandleFunc(middleware.WithCORS("OPTIONS /roles/{id}/share-grants",
		func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(http.StatusNoContent)
		}, opts6))
	mux.HandleFunc(middleware.WithCORS("OPTIONS /roles/{id}/share-grants/{grantId}",
		func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(http.StatusNoContent)
		}, opts6))
}
